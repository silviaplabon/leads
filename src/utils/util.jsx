...
export const FontFamily = `StereoGothic`
...
export const GetTabStyle = (currentMode, modeKey) => ({
  ...
  textAlign: 'center',
  backgroundColor:
    modeKey === 'MyTasks' ? ThemeData.primary : currentMode === modeKey ? ThemeData.primary : '',
...
export const DocumentCategoryForResignationProcess = 'Resignation_Process_Documents'
...
export const nonEditableReqFields = [
  ...
  { name: 'Performance Justification', key: 'performanceJustification', isNotForUser: true },
  {
...
export const GetRequestsOrTasksLovData = (item, activeGroup, userType, isMyTasks = false) => {
  let HRRoles = ''
  const updatedGroupCodeLov = []
  if (userType == 'HR' && item == 'groupCode') {
    for (let i = 0; i < activeGroup?.length; i++) {
      if (activeGroup[i]?.includes('HR')) {
        HRRoles = HRRoles?.length > 0 ? `${HRRoles},${activeGroup[i]}` : activeGroup[i]
...
const isGroupCodeExist = (activeGroup, localData, searchObjKey) => {
  const filteredData = activeGroup?.filter((item) => item?.includes(localData?.groupCode?.label))
...
export const generateSortTextAndCustomQuery = (
  ...
  try {
    ...
    let sortTemp = {}
    if (sortType) {
      let updatedSortType = sortObj[sortType] === 'D' ? 'A' : sortObj[sortType] === 'A' ? '' : 'D'
